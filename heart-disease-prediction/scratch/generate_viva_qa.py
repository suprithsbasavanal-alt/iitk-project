"""
Script to generate reports/Viva_Questions_and_Answers.md with 42 questions & answers.
"""
...
viva_content = """# Capstone Viva Examination: Questions and Answers (Part 9)
...
**Answer**: F1-Score is the harmonic mean of Precision and Recall ($2 \cdot \frac{P \cdot R}{P + R}$), providing a single score that penalizes extreme trade-offs between precision and recall.
...
**Answer**: It standardizes features by subtracting the mean and scaling to unit variance ($z = \frac{x - \mu}{\sigma}$), ensuring features with large numerical scales (e.g., cholesterol) do not dominate distance-based algorithms like KNN or SVM.
...
with open(reports_dir / "Viva_Questions_and_Answers.md", "w") as f:
    f.write(viva_content)

print(f"Generated {reports_dir / 'Viva_Questions_and_Answers.md'}")
