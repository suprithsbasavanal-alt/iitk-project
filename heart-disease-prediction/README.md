...
9. **Final ML Selection**: Freeze the winning Machine Learning model pipeline artifact (`final_model.joblib`).
10. **Deep Learning Integration**: Build, train, and evaluate a multi-layer Keras Artificial Neural Network (`final_ann.keras`) utilizing PyTorch backend execution.
...
12. **Interactive Deployment**: Develop a full-featured Streamlit web application (`app.py`) providing dual-engine inference, input validation, and metric visualizations.
...
9. **Final ML Freezing**: Selected Tuned Random Forest (Test Accuracy: 90.16%, Recall: 96.43%, ROC-AUC: 0.9567, FN=1) and saved pipeline to `models/final/final_model.joblib`.
10. **Deep Learning ANN**: Implemented a 4-layer Keras Artificial Neural Network (ANN-3) with PyTorch backend, achieving 85.25% test accuracy and 96.43% recall (`models/deep_learning/final_ann.keras`).
...
| **Streamlit** | Web Application Framework | Building the interactive web dashboard (`app.py`), session state management, widget UI components, and cloud integration. |
...
The project executed thorough visual and quantitative EDA, outputting **21 publication-quality plots** saved in `results/figures/`:
...
The interactive web application (`app.py`) serves as the production demonstration dashboard:
...
```text
      ...
      (models/final/final_model.joblib)       (models/deep_learning/final_ann.keras)
                    │                                       │
                    └───────────────────┬───────────────────┘
                                        ▼
...
1. **Verify Raw Data**: Confirm `data/raw/heart_disease_uci.csv` matches SHA-256 hash `ecb42eeecad354d241775efcf2084ec2d17c9eb1187498c48408f6d7d5d2834b`.
2. **Execute Full Audit**: Run `python scripts/final_project_audit.py` to execute all 30 automated integrity checks.
3. **Reproduce Baseline Training**: Run `python scripts/train_baseline_models.py`.
4. **Reproduce Tuning**: Run `python scripts/tune_models.py`.
5. **Reproduce ANN Training**: Run `python scripts/train_deep_learning.py`.
...
*For complete citations, DOIs, and full 16-paper summary matrix, view [results/references.md](results/references.md).*
...
- 📄 **[Final Academic Capstone Report](reports/Heart_Disease_Capstone_Final_Report.docx)**: Comprehensive 25+ page academic capstone manuscript (`.docx`).
- 📊 **[Final Presentation Slides](reports/Heart_Disease_Capstone_Presentation.pptx)**: Project presentation slide deck (`.pptx`).
- 💬 **[Viva Questions & Answers](reports/Viva_Questions_and_Answers.md)**: 30 detailed viva defense questions and technical answers.
- 🎬 **[Live Presentation Demo Guide](reports/Demo_Guide.md)**: Step-by-step presentation demonstration guide.
- 📋 **[Submission Manifest](submission/SUBMISSION_MANIFEST.md)**: Master submission verification manifest and artifact hashes.
...
5. **Project Literature Survey**: View complete 16-paper references matrix in [results/references.md](results/references.md).
